...
const UserRow: React.FC<{ user: User; onViewProfile: (userId: string) => void }> = ({
  user,
  onViewProfile,
}) => {
  return (
    <tr className="border-b border-surface-light hover:bg-surface-light/50">
      ...
      <td className="p-3 text-right">
        <button onClick={() => onViewProfile(user.id)} className="text-sm font-semibold text-primary hover:underline">
          View
        </button>
      </td>
    </tr>
...
const FeedbackCard: React.FC<{ feedbackItem: Feedback; onViewProfile: (userId: string) => void }> = ({
  feedbackItem,
  onViewProfile,
}) => {
      ...
      <div className="flex items-start space-x-3">
        ...
        <div className="flex-1">
          <div className="flex items-center space-x-2 text-sm">
            <span className="font-bold cursor-pointer hover:underline" onClick={() => onViewProfile(user.id)}>
              {user.name}
            </span>
...
const Admin: React.FC = () => {
  const {
    ...
    refreshData, // ✅ used by realtime
  } = useAppContext();

  // Restore selected tab when returning from viewing a profile
  const currentEntry: NavigationEntry =
    history.length > 0 ? history[history.length - 1] : { page: 'admin', context: {} };
  const initialTab =
    currentEntry.page === 'admin' && (currentEntry.context as any).adminTab
      ? (currentEntry.context as any).adminTab
      : 'dashboard';

  const [activeTab, setActiveTab] = useState<'dashboard' | 'feedback'>(initialTab);
  const [liveBlink, setLiveBlink] = useState(false); // tiny visual hint when realtime triggers

  // Guard: only master users
  if (!isMasterUser) {
    navigate('feed');
    return null;
  }
  ...
  const TabButton: React.FC<{ label: string; tabName: 'dashboard' | 'feedback' }> = ({ label, tabName }) => (
    <button
      ...
      className={`px-4 py-2 text-sm font-semibold rounded-t-lg transition-colors ${
        activeTab === tabName ? 'bg-surface text-primary border-b-2 border-primary' : 'text-text-secondary hover:bg-surface-light'
      }`}
    >
      {label}
    </button>
  ...
  // ✅ Realtime: refresh when any of these core tables change
  useEffect(() => {
    const unsubs = [
      subscribeToTable('profiles', () => {
        refreshData();
        setLiveBlink(true);
        setTimeout(() => setLiveBlink(false), 600);
      }),
      ...
      subscribeToTable('collaborations', () => {
        refreshData();
        setLiveBlink(true);
        setTimeout(() => setLiveBlink(false), 600);
      }),
      subscribeToTable('feedback', () => {
        refreshData();
        setLiveBlink(true);
        setTimeout(() => setLiveBlink(false), 600);
      }),
  ...
  return (
    <div>
      <header className="app-header flex items-center space-x-4">
        ...
        <div
          className={`ml-2 h-2 w-2 rounded-full ${liveBlink ? 'bg-primary' : 'bg-surface-light'}`}
          title="Live updates"
        />
      </header>
          ...
          <div className="p-4">
            ...
            <div className="bg-surface rounded-lg overflow-x-auto">
              <table className="w-full text-left min-w-full">
                ...
                <tbody>
                  {users.map((user) => (
                    <UserRow key={user.id} user={user} onViewProfile={handleViewProfile} />
                  ))}
                </tbody>
              </table>
            </div>
            ...
            <div className="space-y-4">
              {suggestions.length > 0 ? (
                suggestions.map((suggestion) => (
                  <FeedbackCard key={suggestion.id} feedbackItem={suggestion} onViewProfile={handleViewProfile} />
                ))
              ) : (
