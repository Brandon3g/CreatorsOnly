...
const Collaborations: React.FC<CollaborationsProps> = ({ openCreateModal }) => {
  ...
  // ✅ Realtime: subscribe to `collaborations` table updates
  useEffect(() => {
    const unsub = subscribeToTable('collaborations', () => {
      refreshData();
    });
    return () => {
      try {
        ...
        >
          <div>
            {collaborations.length > 0 ? (
              collaborations.map((collab) => <CollabCard key={collab.id} collab={collab} />)
            ) : (
              <div className="p-8 text-center text-text-secondary">
                <h2 className="text-xl font-bold mb-2">No Opportunities Yet</h2>
                <p>When creators post or update opportunities, they’ll appear here instantly.</p>
              </div>
            )}
          </div>
        </div>
