...
import { Notification, NotificationType, FriendRequestStatus } from '../types';
...
const NotificationItem: React.FC<{ notification: Notification }> = ({ notification }) => {
    const { getUserById, viewProfile, acceptFriendRequest, declineFriendRequest, getFriendRequestById } = useAppContext();
    ...
    const handleNotificationClick = () => {
        switch(notification.type) {
            ...
            case NotificationType.FRIEND_REQUEST_ACCEPTED:
                ...
                break;
            // TODO: Add navigation for other entity types like posts
            default:
                break;
        }
    }
    ...
    const renderContent = () => {
        const actorName = <strong onClick={handleActorClick} className="cursor-pointer hover:underline">{actor.name}</strong>;
        
        switch (notification.type) {
            case NotificationType.FRIEND_REQUEST:
                const request = notification.entityId ? getFriendRequestById(notification.entityId) : undefined;
                ...
                return (
                    <div className="flex-grow">
                        ...
                        <div className="mt-2 space-x-2">
                            <button 
                                onClick={(e) => { e.stopPropagation(); acceptFriendRequest(notification.entityId!)}}
                                onTouchEnd={(e) => { e.stopPropagation(); acceptFriendRequest(notification.entityId!)}}
                                className="bg-primary hover:bg-primary-hover px-4 py-1 text-sm font-semibold text-white rounded-full"
                            >
                                Accept
                            </button>
                            <button 
                                onClick={(e) => { e.stopPropagation(); declineFriendRequest(notification.entityId!)}}
                                onTouchEnd={(e) => { e.stopPropagation(); declineFriendRequest(notification.entityId!)}}
    ...
    return (
        ...
        >
            <div className="text-primary mt-1">
                {notification.type === NotificationType.FRIEND_REQUEST || notification.type === NotificationType.FRIEND_REQUEST_ACCEPTED 
                    ? ICONS.plus 
                    : ICONS.like
                }
            </div>
            <div className="flex-1">
                <img src={actor.avatar} alt={actor.name} className="w-8 h-8 rounded-full mb-2 cursor-pointer" onClick={handleActorClick} />
                {renderContent()}
                <p className="text-xs text-text-secondary mt-1">{timeAgo(notification.timestamp)}</p>
            </div>
...
const NotificationsPage: React.FC = () => {
    const { notifications, currentUser, history, goBack, markNotificationsAsRead, refreshData } = useAppContext();
    const { isRefreshing, pullDistance, isPulling, handlers } = usePullToRefresh({ onRefresh: refreshData });

    const userNotifications = useMemo(() => {
        return notifications
            .filter(n => n.userId === currentUser?.id && n.type !== NotificationType.NEW_MESSAGE)
    ...
    useEffect(() => {
        ...
        if (unreadIds.length > 0) {
            // Mark notifications as read as soon as the page is viewed.
            // A delay was causing issues if the user navigates away before the timeout.
            ...
            <div className="relative">
              ...
              >
                  {userNotifications.length > 0 ? (
                      <div>
                          {userNotifications.map(n => <NotificationItem key={n.id} notification={n} />)}
                      </div>
                  ) : (
                       <div className="p-8 text-center text-text-secondary">
                          <h2 className="text-xl font-bold mb-2">No New Notifications</h2>
                          <p>When you get likes, friend requests, or other notifications, they'll show up here.</p>
                      </div>
                  )}
              </div>
            </div>
