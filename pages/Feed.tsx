...
const Feed: React.FC = () => {
  const { posts, currentUser, users, viewProfile, navigate, isMasterUser, addPost, history, goBack, notifications, refreshData } = useAppContext();
  ...
  const feedPosts = (isMasterUser ? posts : posts
    .filter(post => currentUser.friendIds.includes(post.authorId) || post.authorId === currentUser.id))
    ...
    <div className="w-full">
      <header className="app-header flex items-center space-x-4">
        ...
        <div className="flex-grow flex items-center space-x-4">
            ...
            {isMasterUser && (
                <span className="text-xs font-bold bg-accent-blue text-white px-2 py-1 rounded-full">Master Profile</span>
            )}
        </div>
        <div className="flex-shrink-0 flex items-center space-x-4 md:hidden">
            <button onClick={handleShareApp} onTouchEnd={(e) => { e.preventDefault(); handleShareApp(); }} aria-label="Share App">
                ...
                <div className="flex-1 relative">
                    ...
                    {newPostImage && (
                        <div className="mt-2 relative">
                            <img src={newPostImage} alt="Preview" className="rounded-lg object-contain w-full max-h-80" />
                            <button 
                                onClick={() => setNewPostImage(null)}
                                onTouchEnd={(e) => { e.preventDefault(); setNewPostImage(null); }}
                    ...
                    <div className="flex justify-between items-center mt-2">
                        ...
                        <button 
                            ...
                            onTouchEnd={(e) => { e.preventDefault(); imageInputRef.current?.click(); }}
                        ...
                        <button 
                            ...
                            onTouchEnd={(e) => { e.preventDefault(); handleCreatePost(); }}
