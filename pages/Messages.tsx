...
const Messages: React.FC = () => {
  ...
  // ✅ Realtime: subscribe to messages + conversations and refresh on any change
  useEffect(() => {
    if (!currentUser) return;

    // If you want to filter by row-level data, you can inspect payload.new/payload.old here.
    const unsubMessages = subscribeToTable('messages', () => {
      // Any message change (new, edit, delete) → refresh local store
      refreshData();
      // keep scroll pinned to bottom if you're already viewing the thread:
    ...
    const unsubConversations = subscribeToTable('conversations', () => {
      // Folder changes, lastMessage, participant metadata, etc.
      refreshData();
    });
  ...
  // Keep scrolled to newest message when thread changes
  useEffect(() => {
    if (messagesContainerRef.current) {
      messagesContainerRef.current.scrollTop = messagesContainerRef.current.scrollHeight;
    }
  ...
  // Mark NEW_MESSAGE notifications as read for this conversation when opened
  useEffect(() => {
  ...
  const ConversationListItem: React.FC<{ conv: Conversation; isSelected: boolean }> = ({
    conv,
    isSelected,
  }) => {
    ...
    return (
      <div
        ...
        onTouchStart={() => setSelectedConversationId(conv.id)}
  ...
  const handleSendMessage = () => {
    if (newMessage.trim() && selectedConversationId && currentUser) {
      ...
      setNewMessage('');
      // We'll rely on realtime to echo the new message back;
      // but locally keep scrolled to bottom for snappy UX.
      if (messagesContainerRef.current) {
        setTimeout(() => {
          messagesContainerRef.current!.scrollTop = messagesContainerRef.current!.scrollHeight;
        }, 0);
  ...
  const otherUser =
    selectedConversation && currentUser
      ? getUserById(selectedConversation.participantIds.find((id) => id !== currentUser.id)!)
      : null;

  const FolderButton: React.FC<{ folder: ConversationFolder; label: string }> = ({
    folder,
    label,
  }) => (
    <button
      ...
      className={`flex-1 p-3 text-sm font-bold transition-colors ${
        activeFolder === folder ? 'text-primary border-b-2 border-primary' : 'text-text-secondary hover:text-text-primary'
      }`}
    >
      {label}
    </button>
              ...
              <div className="relative" ref={moveMenuRef}>
                ...
                {isMoveMenuOpen && (
                  <div className="absolute top-8 right-0 bg-surface-light rounded-md shadow-lg z-10 w-48">
                    {selectedConversation.folder !== 'contact_list' && (
                      <button
                        onClick={() => handleMoveConversation('contact_list')}
                        onTouchStart={() => {}}
                    ...
                    {selectedConversation.folder !== 'general' && (
                      <button
                        onClick={() => handleMoveConversation('general')}
                        onTouchStart={() => {}}
                        className="block w-full text-left px-4 py-2 text-sm hover:bg-surface"
                      >
                        Move to General
                    ...
                    {selectedConversation.folder !== 'hidden' && (
                      <button
                        onClick={() => handleMoveConversation('hidden')}
                        onTouchStart={() => {}}
                        className="block w-full text-left px-4 py-2 text-sm hover:bg-surface"
                      >
                        Move to Hidden
                      ...
                      <div
                        className={`max-w-xs lg:max-w-md p-3 rounded-2xl ${
                          msg.senderId === currentUser.id
                            ...
                            : 'bg-surface-light rounded-bl-lg'
                        }`}
                      >
                        <p>{msg.text}</p>
                      </div>
