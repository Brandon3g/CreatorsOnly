...
type NavItemProps = { label: string; iconKey: string; page: Page; notificationCount?: number; };
type MobileNavItemProps = { label?: string; iconKey: string; page: Page; notificationCount?: number; isProfile?: boolean; };
type SidebarProps = { openCreateModal: (initialTab?: 'post' | 'project') => void; onOpenFeedbackModal: () => void; };

const Sidebar: React.FC<SidebarProps> = ({ openCreateModal, onOpenFeedbackModal }) => {
  const {
    currentPage, navigate, currentUser, viewProfile, viewingProfileId, logout,
    isMasterUser, theme, setTheme, notifications,
  } = useAppContext();
  ...
  const applyTheme = (next: 'light' | 'dark') => {
    ...
    try { localStorage.setItem('co-theme', next); } catch {}
  };

  useEffect(() => { setDomTheme(theme === 'dark' ? 'dark' : 'light'); }, [theme]);
  ...
  useEffect(() => {
    ...
    if (!styleEl) {
      ...
      styleEl.textContent = `
        ...
        .ios-fixed { backface-visibility: hidden; transform: translateZ(0); }
      `;
  ...
  const unreadMessageSenders = new Set(
    notifications
      .filter(n => currentUser && n.userId === currentUser.id && n.type === NotificationType.NEW_MESSAGE && !n.isRead)
      .map(n => n.actorId)
  );
  const unreadMessagesCount = unreadMessageSenders.size;

  const otherUnreadCount = notifications.filter(
    n => currentUser && n.userId === currentUser.id && n.type !== NotificationType.NEW_MESSAGE && !n.isRead
  ).length;
  ...
  const NavItem: React.FC<NavItemProps> = ({ label, iconKey, page, notificationCount = 0 }) => {
    ...
    const handleNav = () => { page === 'profile' ? currentUser && viewProfile(currentUser.id) : navigate(page); };
    return (
      <button
      ...
      >
        <div className="relative">
          {ICONS[iconKey]}
          {notificationCount > 0 && (page === 'notifications'
            ? <span className="absolute -top-1 -right-1 block h-2.5 w-2.5 rounded-full bg-accent-red ring-2 ring-background" />
            : <span className="absolute -top-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-accent-red text-white text-xs">{notificationCount > 99 ? '99+' : notificationCount}</span>
          )}
        </div>
  ...
  const MobileNavItem: React.FC<MobileNavItemProps> = ({ label, iconKey, page, notificationCount = 0, isProfile }) => {
    const isActive = page === 'profile'
      ...
      >
        <div className="relative">
          {isProfile && currentUser ? (
            <img src={currentUser.avatar} alt="Profile" className={`h-7 w-7 rounded-full border-2 ${isActive ? 'border-primary' : 'border-transparent'}`} />
          ) : (
            React.cloneElement(ICONS[iconKey] as React.ReactElement<{ className: string }>, { className: 'h-7 w-7' })
          )}
          {notificationCount > 0 && (page === 'notifications'
            ? <span className="absolute top-0 right-0 block h-2 w-2 rounded-full bg-accent-red" />
            : <span className="absolute -top-1 -right-2 flex h-4 w-4 items-center justify-center rounded-full bg-accent-red text-white text-[10px]">{notificationCount > 9 ? '9+' : notificationCount}</span>
          )}
        </div>
      </button>
    );
  ...
  const handleShareApp = async () => {
    ...
    const shareData = { title: 'CreatorsOnly', text: 'Join me on CreatorsOnly, the platform for creators to connect, collaborate, and grow!', url: shareUrl };
    try {
      ...
      <aside className="hidden md:flex fixed top-0 left-0 h-full w-20 lg:w-64 bg-background border-r border-surface-light p-4 flex-col justify-between z-40">
        <div className="w-full">
          ...
          <nav className="mt-8 flex flex-col w-full items-center lg:items-stretch">
            ...
            <button onPointerUp={handleShareApp} className="flex items-center w-full p-3 my-1 rounded-full hover:bg-surface-light transition-colors duration-200">
              <div className="relative">
        ...
        <div className="w-full">
          <button onPointerUp={() => openCreateModal('post')} className="w-full bg-primary hover:bg-primary-hover text-white font-bold py-3 px-4 rounded-full hidden lg:block">Create</button>

          <button onPointerUp={() => openCreateModal('post')} className="w-12 h-12 bg-primary hover:bg-primary-hover text-white font-bold rounded-full flex items-center justify-center lg:hidden" aria-label="Create">
          ...
          <div className="mt-4 relative" ref={settingsRef}>
            ...
            {isSettingsOpen && (
              <div role="menu" className="absolute bottom-full mb-2 w-full rounded-xl shadow-2xl z-50 border border-surface overflow-hidden bg-surface" style={{ backgroundColor: 'var(--co-surface-opaque-color)' }}>
                <div className="flex items-center justify-between px-4 py-3">
                  ...
                  <div className="flex items-center gap-1 rounded-full bg-surface p-0.5">
                    <button type="button" aria-label="Light theme" className={`h-7 w-7 rounded-full flex items-center justify-center ${theme === 'light' ? 'bg-primary text-white' : 'text-text-secondary'}`} onPointerUp={() => applyTheme('light')}>
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z"/>
                      </svg>
                    </button>
                    <button type="button" aria-label="Dark theme" className={`h-7 w-7 rounded-full flex items-center justify-center ${theme === 'dark' ? 'bg-primary text-white' : 'text-text-secondary'}`} onPointerUp={() => applyTheme('dark')}>
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <button type="button" onPointerUp={() => { onOpenFeedbackModal(); setIsSettingsOpen(false); }} className="w-full text-left px-4 py-3 text-sm text-text-primary hover:bg-surface-light" role="menuitem">
                  Send Feedback
                </button>

                <div className="border-t border-surface/70" />

                <button type="button" onPointerUp={() => { logout(); setIsSettingsOpen(false); }} className="w-full text-left px-4 py-3 text-sm text-primary hover:bg-surface-light" role="menuitem">
