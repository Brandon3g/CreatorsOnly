...
const Sidebar: React.FC<SidebarProps> = ({ openCreateModal, onOpenFeedbackModal }) => {
  const { currentPage, navigate, currentUser, viewProfile, viewingProfileId, logout, isMasterUser, theme, setTheme, notifications } = useAppContext();
  ...
  const otherUnreadCount = notifications.filter(n => currentUser && n.userId === currentUser.id && n.type !== NotificationType.NEW_MESSAGE && !n.isRead).length;
  ...
  const MobileNavItem: React.FC<MobileNavItemProps> = ({ iconKey, page, notificationCount = 0, isProfile }) => {
    ...
    <>
        ...
        <div className="hidden md:flex fixed top-0 left-0 h-full w-20 lg:w-64 bg-background border-r border-surface-light p-4 flex-col justify-between z-40">
            <div className="w-full">
                <div className="text-primary font-bold text-2xl p-3 hidden lg:block">
                    CreatorsOnly
                </div>
                ...
                <nav className="mt-8 flex flex-col w-full items-center lg:items-stretch">
                    ...
                    <button
                        ...
                        onTouchEnd={(e) => { e.preventDefault(); handleShareApp(); }}
            ...
            <div className="w-full">
                <button onClick={() => openCreateModal('post')} onTouchEnd={(e) => { e.preventDefault(); openCreateModal('post'); }} className="w-full bg-primary hover:bg-primary-hover text-white font-bold py-3 px-4 rounded-full hidden lg:block">
                    Create
                </button>
                <button onClick={() => openCreateModal('post')} onTouchEnd={(e) => { e.preventDefault(); openCreateModal('post'); }} className="w-12 h-12 bg-primary hover:bg-primary-hover text-white font-bold rounded-full flex items-center justify-center lg:hidden">
                ...
                <div className="mt-4 relative" ref={settingsRef}>
                    {isSettingsOpen && (
                        <div className="absolute bottom-full mb-2 w-full bg-surface-light rounded-lg shadow-lg py-1 z-50">
                            ...
                            <button 
                                onClick={() => { onOpenFeedbackModal(); setIsSettingsOpen(false); }}
                                onTouchEnd={(e) => { e.preventDefault(); onOpenFeedbackModal(); setIsSettingsOpen(false); }}
                                className="w-full text-left px-4 py-2 text-sm text-text-primary hover:bg-surface"
                            >
                                Send Feedback
                            </button>
                            <button 
                                onClick={() => { logout(); setIsSettingsOpen(false); }}
                                onTouchEnd={(e) => { e.preventDefault(); logout(); setIsSettingsOpen(false); }}
                                className="w-full text-left px-4 py-2 text-sm text-text-primary hover:bg-surface"
                            >
                    ...
                    <button 
                        ...
                        onTouchEnd={(e) => { e.preventDefault(); setIsSettingsOpen(!isSettingsOpen); }}
                        className="w-full flex items-center justify-between text-left p-2 rounded-full hover:bg-surface-light"
                        aria-haspopup="true"
        ...
        <nav className="md:hidden fixed bottom-0 left-0 w-full bg-background border-t border-surface-light z-50 flex justify-around items-center px-2 mobile-nav-safe-area">
            <MobileNavItem iconKey="home" page="feed" />
            <MobileNavItem iconKey="explore" page="explore" />
            <MobileNavItem iconKey="collaborations" page="collaborations" />
            <MobileNavItem iconKey="messages" page="messages" notificationCount={unreadMessagesCount} />
            {isMasterUser && <MobileNavItem iconKey="settings" page="admin" />}
            <MobileNavItem isProfile={true} iconKey="profile" page="profile" />
        </nav>
    </>
  );
};
